...
// steer/servo params
#define MAX_STEER_LEFT -0.34                       // value determined by demo mode 1 measure (have to be adjusted with every servo horn attach)
#define MAX_STEER_RIGHT 0.34                       // value determined by demo mode 1 measure
...
#define CONTROL_METHOD 0                           // which control method to use
...
#define MIN_POWER 30                               // percent min power (estimating for a 2-ft turn => 24" / (24" + 6" car) = 4/5; speed of inner wheel is 20% lower worst case
...
void MasterControlProgram()
{
  ...
  if ((TFC_Ticker[0] > 2000 || (terminalOutput != 3)) && (TFC_LineScanImageReady > 0 || debugFakeMode))
  {
    ...
    // facem derivata si aflam min/max derivata
    derivativeLineScan(&GrabLineScanImage0[0], &DerivLineScanImage0[0]);

    // ajustam threshold in functie de maximul/minimul de la derivateLineScan
    adjustLights();
